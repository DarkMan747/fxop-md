...
const PausedChats = require("../Stores/paused");
const config = require("../../../config");
const { serialize } = require("./Serials");
const { Greetings, sessionID } = require("../Utils");
const { Image, Message, Sticker, Video, AllMessage } = require("../Class");
const { loadMessage, saveMessage, saveChat, getName } = require("../Stores/store");
const plugins = require("../Utils");
const sys = require("./Patch");
const logger = pino({ level: process.env.LOG_LEVEL || "silent" });

const client = async () => {
  (async () => { const clog = console.log; console.log = () => {}; await sys.patch(); console.log = clog; })();
 ...
 const handleConnection = async ({ connection, lastDisconnect }) => {
  if (connection === "open") {
   ...
   const aliveMsg = `FXOPRISA MD ${packageVersion}\nPREFIX: ${config.HANDLERS}\nPLUGINS: ${plugins.commands.length}\nMODE: ${config.WORK_TYPE}`;
