...
const requireJS = async (dir, { recursive = false, fileFilter = f => path.extname(f) === ".js" } = {}) => Promise.all((recursive ? (await Promise.all((await fs.readdir(dir, { withFileTypes: true })).map(e => path.resolve(dir, e.name)).map(p => fs.stat(p).then(s => (s.isDirectory() ? requireJS(p, { recursive, fileFilter }) : p))))).flat() : await fs.readdir(dir)).filter(fileFilter).map(f => require(path.isAbsolute(f) ? f : path.join(dir, f)).catch(() => null))).then(m => m.filter(Boolean));
